...
export function generateCloudflareWorkerFiles(input) {
...
function createWorkerSource(ir) {
    const routeLines = ir.handlers.length === 0
        ...
        : ir.handlers.map((handler) => `app.${handler.method.toLowerCase()}(${JSON.stringify(handler.path)}, (c) => c.text(${JSON.stringify(handler.responseText)}));`);
    return [
