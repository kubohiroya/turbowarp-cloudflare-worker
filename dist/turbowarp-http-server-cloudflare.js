...
(function (Scratch) {
  ...
  var extensionConfig = {
  	id: "kubohiroyahttpservercloudflare",
  	slug: "turbowarp-http-server-cloudflare",
  	name: "TurboWarp HTTP Server Cloudflare",
  	description: "Cloudflare Workers generation blocks for turbowarp-http-server.",
  	author: "Hiroya Kubo",
  	license: "MPL-2.0",
  ...
  function createWorkerSource(ir) {
  	const routeLines = ir.handlers.length === 0 ? ["app.get('/', (c) => c.text('Generated by turbowarp-http-server-cloudflare.'));"] : ir.handlers.map((handler) => `app.${handler.method.toLowerCase()}(${JSON.stringify(handler.path)}, (c) => c.text(${JSON.stringify(handler.responseText)}));`);
  	return [
